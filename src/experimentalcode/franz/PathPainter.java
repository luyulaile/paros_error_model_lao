 ...
 * @author graf
 */
class PathPainter extends AbstractPainter<JXMapViewer> {
    ...
    void setPath(Path<?, ? extends OSMNode> path) {
        this.paths = new ArrayList<Path<?, ? extends OSMNode>>(1);
        this.paths.add(path);
    }

    void setPath(List<Path<?, ? extends OSMNode>> pathList) {
        this.paths = pathList;
    }
    ...
    private void paintPath(Path<?, ? extends OSMNode> path, TileFactory tf, int zoom, Rectangle2D vp, Graphics2D g) {
        ...
        for (int i = 0; i < list.size() - 1; i++) {
            ...
            if (!vp.contains(srcPoint) && !vp.contains(dstPoint)) {
                continue;
            }
            {
